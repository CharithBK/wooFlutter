...
import 'package:ecom/listViews/cart_wishlist_horizontal_listview.dart';
...
class _CartScreenState extends State<CartScreen>  {
  ...
  Widget body() {
    return Container(
      child:widget.showAppBar? Column(
              ...
              child: Column(
                children: [
                  Stack(
                    children: [
                      Container(
                        height: (MediaQuery.of(context).size.height * 0.65) - (showWishList ? wishListHeight : 0),
                  ...
                  showWishList ? Container(
                      height: wishListHeight,
                      child: Stack(
                        children: [
                          WishListWidget().getWishListView(context, showWishList, (){setState(() {});}),
                          Align(
                              alignment: Alignment.topRight,
                              child: Stack(
                                children: [
                                  IconButton(color: SecondaryHighlighter, icon: Icon(Icons.circle, color: BackgroundColor,), iconSize: 30, onPressed: (){},),
                                  IconButton(color: SecondaryHighlighter, icon: Icon(Icons.cancel, color: MainHighlighter,), iconSize: 30, onPressed: (){
                                    showWishList = false;
                                    setState(() {});
                                  },)
                                ],
  ...
  Widget cartItems(Line_items item,int index) {
    ...
    if(couponSelection==CouponSelection.Accepted && _coupon!=null &&
        (!_coupon!.exclude_sale_items||(_coupon!.exclude_sale_items && !getproduct.on_sale))){
        {
          bool couponAppicableOnProduct=isValidTokenForProduct(_coupon!,getproduct);
          bool couponAppicableOnCateogory=isValidTokenForP_Category(_coupon!,getproduct.categories);
                    ...
                    child: Row(
                      ...
                      children: <Widget>[
                        Container(
                          ...
                          child: Text(
                            "${getproduct!=null? getproduct.title!=null?getproduct.title:"" :""}".toUpperCase(),
  ...
  Widget _checkoutSection(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          Container(height: 1,color: NormalColor,),
          Stack(
            children: [
              StreamBuilder(
                ...
                builder: (BuildContext context, AsyncSnapshot snapshot) {
                  return snapshot.data!=null?getCoupon():
                  Center(child: JumpingDotsProgressIndicator(fontSize: 30.0, ));
                },
              )
            ],
          ),
          ...
          GestureDetector(
            onTap: () {
              if (totalCost > 0) {
                ...
                ToastUtils.showCustomToast(context, "Cart price must not be zero");
      ...
      return Flex(
        ...
        children: [
          ...
          Expanded(
              ...
              child: GestureDetector(
                onTap: () {
                  _coupon = coupons!.firstWhereOrNull((element) => element.code
                      .toLowerCase() == couponController.text.toLowerCase());
                  if (_coupon != null) {

                    double minimum_amount = double.parse(
                        _coupon!.minimum_amount != null &&
                            _coupon!.minimum_amount != "" ? _coupon!.minimum_amount : "0.0");
                    double maximum_amount = double.parse(
                        _coupon!.maximum_amount != null &&
                            _coupon!.maximum_amount != "" ? _coupon!.maximum_amount : "0.0");
                    bool inAmountInCouponRange = totalCost > minimum_amount &&
                        totalCost < maximum_amount;
                    bool isDateNotPassed = _coupon!.date_expires==null?true:DateTime.now().isBefore(
                        DateTime.parse(_coupon!.date_expires).toLocal());
                    bool isUsageInLimit = _coupon!.usage_count < _coupon!.usage_limit;


                    print("");
                    if (isDateNotPassed && isUsageInLimit && inAmountInCouponRange) {
